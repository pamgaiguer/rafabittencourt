...
class MPTweetList extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		if ($user1 != '') {
			...
			$user1_data = file_get_contents_curl($user1_url);
		...
		if ($user2 != '') {
			...
			$user2_data = file_get_contents_curl($user2_url);
			$tweet_pattern = "/<text>(.+)<\/text>/";
			...
			while ($m < $max) {
				if ($convert_links == 'yes') {
					$user2_full_tweet = preg_replace("/(https?:\/\/\w+\.\w+\/\w+)/","<a href='$1' target='_blank'>$1</a> ",$user2_recent[0][$m]);
				}
				if ($convert_users == 'yes') {
					if ($convert_links == 'no') { $user2_full_tweet = $user2_recent[0][$m]; }
				...
				if ($convert_links != 'yes' && $convert_users != 'yes' && $convert_hashs != 'yes') {
						$user2_tweets .= '<li>'.$user2_recent[0][$m].'</li>';
				} else {
